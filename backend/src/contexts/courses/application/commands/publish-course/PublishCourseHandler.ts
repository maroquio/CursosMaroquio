...
import { PublishCourseCommand } from './PublishCourseCommand.ts';
 ...
 * Handles course publication
 */
export class PublishCourseHandler implements ICommandHandler<PublishCourseCommand, CourseDto> {
  constructor(private courseRepository: ICourseRepository) {}

  async execute(command: PublishCourseCommand): Promise<Result<CourseDto>> {
    // Validate course ID
    ...
    if (publishResult.isFailure) {
      return Result.fail(publishResult.getError() as string);
    }

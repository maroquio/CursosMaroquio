...
/**
 * Strongly-typed Module ID value object
 * Prevents accidental confusion with other ID types
 */
export class ModuleId extends Identifier<ModuleId> {
  private constructor(value: string) {
    super(value);
  }

  /**
   * Create a new ModuleId with UUID v7
   */
  public static create(id?: string): ModuleId {
    return new ModuleId(id ?? uuidv7());
  }

  /**
   * Create a ModuleId from an existing string
   * Validates UUID v7 format
   */
  public static createFromString(id: string): Result<ModuleId> {
    if (!this.isValidUUIDv7(id)) {
      return Result.fail(ErrorCode.INVALID_MODULE_ID);
    }
    return Result.ok(new ModuleId(id));
  }
